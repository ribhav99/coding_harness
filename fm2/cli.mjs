...
if (command === 'review') {
  const number = process.argv[3] ?? die('usage: fm review <pr-number> [--project <dir>] [--window <name>] [--agent claude|codex]');
...
if (command === 'status') {
  ...
  for (const task of tasks) {
    ...
    process.stdout.write(`${task.id}\t${task.pane}\t${alive}${where}${forge}${provider}${muted}${done}\n`);
