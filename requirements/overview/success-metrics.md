...
Autonomous loops converge. A typical run passes within a bounded number of attempts. Exhaustion — hitting the retry cap — is rare. When exhaustion does happen, the fix is almost always to improve the input artifact (tighten the PRD, answer an open clarification question, amend a blueprint), not to patch the harness itself.
...
Per-loop operator involvement stays bounded. The operator answers open clarification questions in `_questions-pending.md` for the upstream loops and reviews PRs for the coding loop, but does not do the authoring, decomposition, or scoping work the loops are meant to automate. If open-question frequency or PR rework climbs, the loops have lost their leverage.
