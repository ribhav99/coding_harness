...
Product decisions follow from this persona. Polish is scoped to what one operator needs for long-term reuse on their own projects. There is no second persona to accommodate — no team lead, no reviewer, no downstream consumer. The operator authors the PRD, answers open clarification questions in `_questions-pending.md` for each upstream loop, reviews and merges PRs, and maintains the harness itself.
