...
- **AC-ORCH-012.5** — The orchestrator shall not write inside the agent files in the communication folder. Generator and reviewer subprocesses are the only writers; the orchestrator manages folder-level lifecycle (wipe / snapshot) only.
