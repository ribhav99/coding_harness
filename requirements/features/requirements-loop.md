...
The Requirements Loop is the first autonomous stage after PRD authoring. It reads the monolithic `PRD.md` at the project repo's root and decomposes it into the full structural requirements tree — product-overview sections under `requirements/overview/` and Feature Requirements Documents under `requirements/features/`. An autonomous generator writes the tree; five reviewer subprocesses evaluate it against distinct rubrics; the orchestrator re-spawns the generator with aggregated feedback on any fail and commits the tree on all-pass.
...
- **Dual completion condition** — the pass condition for this loop: all five reviewers pass AND `requirements/_questions-pending.md` has no open questions blocking further review.
...
### REQ-RL-006 — Five reviewer subprocesses
**User Story.** As an operator, I want the requirements tree checked against five distinct rubrics, so that defects in structure, cross-doc consistency, PRD topic-coverage, feature scoping, and PRD-fidelity (the REQ/AC/OOS contents inside each node, not just the topic shape) are all caught before the tree is committed.
...
- **AC-RL-006.3** — The orchestrator shall spawn `req-coverage-judge` to verify the union of FRDs covers the PRD's scope without gaps or overlaps, reading both `requirements/overview/` and `requirements/features/`. This judge checks *topic-level* parity (every PRD topic has a node; no node is fabricated).
...
- **AC-RL-006.5** — The orchestrator shall spawn `req-prd-fidelity-judge` to verify the bodies of the overview docs and FRDs faithfully reflect the body of `PRD.md`: every PRD-defined `REQ-<KEY>-<NUMBER>` has a matching REQ in the right tree node (`MISSING_REQ_IN_TREE`); PRD `OUT OF SCOPE` markers are honored in the tree (`STALE_OOS` / `FABRICATED_OOS`); new ACs the PRD adds to existing REQs appear in the tree (`MISSING_AC`); PRD vocabulary updates are reflected consistently (`TERMINOLOGY_DRIFT`); REQs in the tree without a PRD source are flagged (`FABRICATED_REQ`). This is the inside-the-node companion to `req-coverage-judge`'s topic-level check.
- **AC-RL-006.6** — Each reviewer subprocess shall run in a fresh Claude Code context with `--disallowedTools Bash,NotebookEdit`. `Write` and `Edit` are allowed so the reviewer can append its review to its own communication file (§REQ-RL-011); a `PreToolUse` path-guard hook shall block any `Write`/`Edit` call whose target path is not exactly the reviewer's own `requirements_communication/<reviewer-name>.md` file.
- **AC-RL-006.7** — Each reviewer shall append a `## Review — attempt N` block to its communication file, ending with a single trailing line matching exactly `VERDICT: pass` or `VERDICT: fail`. The reviewer's stdout shall be a short acknowledgement ending with the same `VERDICT:` line — that's the only signal the orchestrator parses for loop control.
- **AC-RL-006.8** — At every attempt boundary and on every loop-exit verdict (`pass`, `awaiting_clarification`, `exhausted`), the orchestrator shall snapshot each reviewer's communication file from `requirements_communication/` into `harness/state/reviews/requirements-loop/attempt-<N>/<reviewer-name>.md`.
...
- **AC-RL-008.1** — Full `pass` shall require both all five reviewers pass AND `requirements/_questions-pending.md` has no open questions blocking the review.
...
- **AC-RL-011.2** — The communication folder shall contain one markdown file per agent in the loop: `prd-to-frds.md` (generator outbound) plus one per reviewer (`req-spec-judge.md`, `req-cross-doc-judge.md`, `req-coverage-judge.md`, `req-scoping-judge.md`, `req-prd-fidelity-judge.md`).
- **AC-RL-011.3** — The orchestrator shall ensure the communication folder exists before spawning the generator (mkdir-with-exists-ok). The folder shall never be wiped — it accumulates the full reviewer-generator conversation across attempts and across invocations forever.
...
- **AC-RL-011.7** — On every loop-exit verdict (`pass`, `awaiting_clarification`, `exhausted`), the live communication folder shall be left in place. Re-running the loop after any exit verdict picks up the prior conversation as context for the next attempt; the per-attempt snapshots under `harness/state/reviews/<loop>/attempt-<N>/` provide the frozen audit trail alongside.
...
The five reviewers are designed to prevent distinct downstream failures. `req-spec-judge` catches structurally-malformed FRDs that would confuse the blueprint generator. `req-cross-doc-judge` catches terminology drift and duplication that would let the blueprint generator produce inconsistent blueprints. `req-coverage-judge` catches PRD topic-scope that did not make it into any FRD — the topic-level load-bearing check. `req-scoping-judge` catches FRDs that do not pass the feature-unit definition and would therefore produce blueprints that do not decompose cleanly into work orders. `req-prd-fidelity-judge` catches drift *inside* each node — REQs the PRD added but the tree missed, OOS markers the PRD set that the tree didn't honor, renames the PRD made that the tree didn't propagate. Coverage and fidelity are companions: coverage asks "is every topic shaped?"; fidelity asks "is every REQ inside the topic faithful to the PRD?". Each reviewer is focused; none repeats another's work.

The communication folder is what makes the gen↔reviewer back-and-forth visible and durable. Without it, the orchestrator would have to assemble retry-context prompt blocks every attempt and the generator's push-backs would only persist as inlined prose. With it, the conversation accumulates across attempts and across invocations in markdown files the operator can read directly, and the generator can push back on a specific reviewer by appending to that reviewer's file — the reviewer sees the push-back on its next run and may revise its position. The orchestrator stays small: spawn subprocesses, parse `VERDICT:` lines, snapshot at attempt boundaries, never wipe.
