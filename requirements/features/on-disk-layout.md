...
- **AC-LAYOUT-005.8** — `work-orders/_inbox/` shall hold gaps filed by the coding loop; gap entries are flat `wo-<slug>.md` files in the same shape, queued for operator triage. The operator triages and either promotes a gap into the main `work-orders/` tree or discards it.
