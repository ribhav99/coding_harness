...
- **Communication folder** — `requirements_communication/`, `blueprints_communication/`, or `work-orders_communication/` at the project root, sibling to the corresponding artifact tree. Holds one markdown file per agent in that loop (one for the generator, one per reviewer). Append-only conversation transcript; both the generator and the named reviewer read and write the file. Orchestrator manages folder-level lifecycle (ensure / snapshot) only — the folder is never wiped, accumulating the full reviewer-generator conversation across attempts and across invocations forever.
...
### REQ-LAYOUT-005 — Flat slug-based work-orders tree
**User Story.** As an operator, I want work orders as flat files at the work-orders root with stable slug-based names and a separate sequence file, so that re-ordering and editing don't require renaming directories or breaking cross-references.
- **AC-LAYOUT-005.1** — `work-orders/` shall hold work orders as flat files at the top level; no per-WO subdirectories, no phase groupings, no `wo-NNN` numbering.
- **AC-LAYOUT-005.2** — Each work order shall consist of two files: a visible `wo-<slug>.md` (the scoped-task body) and a sibling dotted-hidden `.wo-<slug>.meta.yaml` (orchestrator-managed). The slug is a stable kebab-case identifier (e.g. `wo-add-signin-endpoint`); once chosen, a slug is never renamed (renaming a slug breaks every cross-reference that points to it).
- **AC-LAYOUT-005.3** — `.wo-<slug>.meta.yaml` shall carry: `id` (= the slug), `status` (`backlog | ready | in_progress | done | blocked_external`), `priority`, `type` (`feature | refactor | bug-fix | infra | operator-action`), `parent_id`, `blocked_by[]`, `blueprint_ids[]`. There is no `sort_order` field on the meta file; ordering lives in `_sequence.md` (AC-LAYOUT-005.5).
- **AC-LAYOUT-005.4** — The per-work-order document shape of `wo-<slug>.md` (sections, mention syntax, structured fenced YAML blocks for `Produces`/`Depends on`/`Gates`, acceptance-criteria schema) is pinned in the work-orders-loop FRD (`requirements/features/work-orders-loop.md` REQ-WO-002); this on-disk-layout FRD covers only the file naming and meta-file structure.
- **AC-LAYOUT-005.5** — `work-orders/_sequence.md` shall list every work-order slug in execution order, top to bottom, as a numbered markdown list with each entry as a backticked `wo-<slug>` ID. The sequence is mutable — the operator may re-order it before re-running the work-orders-loop. The orchestrator's `pick_next` walks `_sequence.md` top to bottom and selects the first work order whose `status: ready`, whose `blocked_by[]` are all `done`, and whose `type` is not `operator-action`.
- **AC-LAYOUT-005.6** — `work-orders/_external-blockers.md` shall be present whenever any work order has `type: operator-action` (with `status` not `done`) OR `status: blocked_external`. The file is operator-readable and lists every work order requiring operator action; it is regenerated by the orchestrator on every loop-exit verdict.
- **AC-LAYOUT-005.7** — `work-orders/.sequence.meta.yaml` shall record the blueprint-tree hash at the time the sequence was generated plus a generation timestamp.
- **AC-LAYOUT-005.8** — `work-orders/_backlog/` shall hold gaps filed by the coding loop; gap entries are flat `wo-<slug>.md` files in the same shape, queued for operator triage. The operator triages and either promotes a gap into the main `work-orders/` tree or discards it.
- **AC-LAYOUT-005.9** — The orchestrator shall materialise `.wo-<slug>.meta.yaml.blocked_by[]` from the `Depends on.work_orders` list inside the work order's `wo-<slug>.md`; the description is the single source of truth for dependencies.
...
- **AC-LAYOUT-012.2** — Each communication folder shall contain one markdown file per agent in its loop: one file for the generator (e.g. `prd-to-frds.md`, `frd-to-blueprint.md`, `blueprint-to-work-orders.md`) and one per reviewer (e.g. `req-spec-judge.md`, `bp-spec-judge.md`, `wo-spec-judge.md`).
...
- **AC-LAYOUT-012.4** — The orchestrator shall ensure the communication folder exists before spawning the generator (mkdir-with-exists-ok). The folder shall never be wiped by the orchestrator — it accumulates the full reviewer-generator conversation across attempts and across invocations forever.
- **AC-LAYOUT-012.5** — At every attempt boundary and on every loop-exit verdict, the orchestrator shall snapshot each agent file from the live communication folder into `harness/state/reviews/<loop>/attempt-<N>/<reviewer-name>.md`. The per-attempt archive is the frozen audit record; the live folder is preserved as the running conversation.
- **AC-LAYOUT-012.6** — On every loop-exit verdict (`pass`, `awaiting_clarification`, `exhausted`), the live communication folder shall be left in place. Re-running a loop after any exit verdict picks up the prior conversation as context for the next attempt.
...
- **AC-LAYOUT-008.1** — `harness/state/` shall hold JSON state files: one per loop (e.g. `requirements-loop.json`, `blueprint-loop.json`, `work-orders-loop.json`) and one per in-progress work order (e.g. `<wo-slug>.json`).
- **AC-LAYOUT-008.2** — `harness/state/reviews/<loop>/[<wo-slug>/]attempt-<N>/<reviewer-name>.md` shall archive each reviewer's review. For upstream loops (requirements, blueprint, work-orders), the archive is a snapshot of the reviewer's communication file at the attempt boundary. For coding-loop per-work-order execution, the archive is the captured stdout of the reviewer subprocess.
- **AC-LAYOUT-008.3** — `harness/logs/<wo-slug>/` shall hold hook and session logs.
...
Work orders are flat slug-named files, no phases, no per-WO directories. The Software Factory model uses phase groupings to give humans a mental model of when things run; the harness's work orders are agent-consumed and do not need that. `_sequence.md` (a mutable numbered list of work-order slugs) gives the execution order; each work order's `Depends on.work_orders` plus the materialised `.wo-<slug>.meta.yaml.blocked_by[]` encode dependencies. The orchestrator's `pick_next` walks `_sequence.md` top to bottom, returning the first work order whose `status: ready`, `blocked_by[]` all `done`, and `type` not `operator-action`. Slug-based stable IDs mean the operator can re-order `_sequence.md` without renaming files or breaking cross-references.
