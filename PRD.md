...
2. **Requirements Loop** (upstream, autonomous, with non-blocking clarification questions). Reads the root `PRD.md` and decomposes it into the full structural tree: product-overview sections (business problem, personas, product description, success metrics, etc. — the `requirements/overview/` tree) *and* Feature Requirements Documents (`requirements/features/` tree). Reviews against five rubrics (spec structure, cross-doc consistency, topic-level PRD coverage, feature scoping, PRD fidelity inside each node) and iterates until all pass. When the PRD itself is ambiguous or contradictory, the generator logs questions to `requirements/_questions-pending.md` and keeps working on everything else; the operator resolves by editing `PRD.md`.
...
**Reviewers.** Orchestrator spawns five reviewer subprocesses after the generator exits: structural spec, cross-doc consistency, topic-level PRD coverage, feature scoping, and PRD fidelity inside each node. See §8.1 for rubric detail.
...
**Reviewer ↔ generator communication folder.** Live conversation between the generator and the five reviewers happens in `blueprints_communication/` at the project root (sibling to `blueprints/`). One file per reviewer (`bp-spec-judge.md`, `bp-coverage-judge.md`, `bp-consistency-judge.md`, `bp-decision-judge.md`, `bp-frd-fidelity-judge.md`). Append-only conversation transcripts; both sides read and write. The generator pushes back on findings it disagrees with by appending to the relevant reviewer's file; the reviewer reads the push-back on its next run and may revise its position. Race-free because the orchestrator runs gen→reviewers strictly sequentially per attempt, and reviewers each write only their own file. See `BLUEPRINT.md §1.8` for the full lifecycle (preserved across invocations and never wiped, snapshot to `harness/state/reviews/blueprint-loop/attempt-<N>/` at attempt boundaries — the audit dir is the frozen per-attempt record, the live folder is the running conversation).

**Reviewers.** Orchestrator spawns five: structural spec (`bp-spec-judge`), slug-level FRD-to-blueprint coverage (`bp-coverage-judge`), cross-blueprint consistency (`bp-consistency-judge`), decision hygiene (`bp-decision-judge`), and FRD fidelity inside each feature blueprint (`bp-frd-fidelity-judge`). See §8.2 for rubric detail.
...
**Generator.** A single `claude -p` subprocess running with the `blueprint-to-work-orders` skill. It reads `blueprints/`, the existing `work-orders/` tree, and the project's existing source code (everything outside the harness-managed trees `requirements/`, `blueprints/`, `work-orders/`, `harness/`, `*_communication/`), and produces a flat tree of slug-named work orders under `work-orders/`, plus a separate `_sequence.md` recording the execution order. No phase groupings, no per-WO directories, no `wo-NNN` numbering — work orders live as flat `wo-<slug>.md` + `.wo-<slug>.meta.yaml` pairs. The codebase scan is targeted (top-level structure + language/framework config; entry-point files named in container blueprints; greps for component/model/route names declared in blueprints) — not exhaustive. Existing work orders are read first so partial regeneration works; on every invocation the generator compares the current blueprint-tree hash against the hash recorded in `work-orders/.sequence.meta.yaml` and short-circuits if blueprints are unchanged and there are no failing reviews to address. **Decision rule for the generator:** for each blueprint surface element, if the code already realizes it, no work order is created (the code is the coverage); if the code partially realizes it, a work order scoped to the gap; if the code doesn't realize it, a full work order. Material divergence between code and blueprint (the symbol exists with a different shape than the blueprint declares) is logged as a clarification question rather than silently chosen.
...
**Playwright gate opt-in.** The Playwright gate is *opt-in per coding-loop invocation* via `--with-playwright`. Default off. Without the flag, the orchestrator skips spawning `playwright-runner` regardless of any WO's `## Gates` declaration; with the flag, the Gates block governs as normal. Rationale: Playwright is the only gate that boots a long-running process (dev server) and runs a real browser suite, so each invocation that includes it pays a minute-class cost — that cost is worth paying when validating UI behavior end-to-end, and not when iterating on logic-only changes. The flag lives at the loop level, not per-WO, because the operator is the one who knows whether the project is currently in a runnable state.
...
- `req-coverage-judge` *(reviewer, LLM-as-judge)* — topic-level parity: union of FRDs covers the PRD's scope without gaps or overlaps.
...
- `req-prd-fidelity-judge` *(reviewer, LLM-as-judge)* — drift inside each tree node: every PRD-defined REQ appears in the right node with matching User Story and ACs; PRD OOS markers honored; AC additions propagate; PRD vocabulary updates reflected; no fabricated REQs.
...
- `bp-spec-judge`, `bp-coverage-judge`, `bp-consistency-judge`, `bp-decision-judge`, `bp-frd-fidelity-judge` *(reviewers, LLM-as-judge)* — per §6.3 / §8.2. Each reads its own communication file in `blueprints_communication/<reviewer-name>.md`, runs its review, and appends the review block to that same file. The decision-judge verifies (a) no high-impact decision was silently made by the generator without writing a question block, and (b) any unresolved architectural choice has a matching open block in `blueprints/_questions-pending.md`. The fidelity-judge verifies each feature blueprint's body faithfully reflects its FRD's body (REQ coverage, OOS alignment, terminology consistency) — companion to coverage-judge's slug-level check.

**Work-orders-loop skills** (autonomous):

- `blueprint-to-work-orders` *(generator, LLM work)* — sole loop generator. Reads `blueprints/`, the existing `work-orders/` tree (every `wo-<slug>.md` and `_sequence.md`), `work-orders/_questions-pending.md`, `work-orders/.sequence.meta.yaml`, and the project's existing source code (everything outside the harness-managed trees `requirements/`, `blueprints/`, `work-orders/`, `harness/`, `*_communication/`) via a targeted scan (top-level structure + config; entry-point files named in container blueprints; greps for blueprint-named symbols, models, and routes). Applies the realised-vs-partial-vs-missing decision rule to skip work orders for fully-realised blueprint surface and scope work orders to gaps for partial realisations. Produces the full work-order tree as flat slug-named files plus a `_sequence.md` recording execution order, writing each `wo-<slug>.md` directly in the canonical document shape (§6.4 / `requirements/features/work-orders-loop.md` REQ-WO-002). Marks operator-action work orders with `type: operator-action` in their meta when the work is something the operator must do themselves. No separate "stub then expand" step. Reads each reviewer's communication file in `work-orders_communication/` and appends per-finding responses (fix / push back / surface to operator) plus a change-summary on retries. Files bare-question blocks to `work-orders/_questions-pending.md` for decomposition ambiguities and for material divergence between code and blueprint shape.
...
### 8.1 Requirements loop (5 gates + dual completion condition)
...
- **`req-coverage-judge`** — union of FRDs covers the PRD's *topic scope* without gaps or overlaps. Reads `requirements/overview/` and `requirements/features/`. Topic-level parity (every PRD topic has a node; no node is fabricated).
...
- **`req-prd-fidelity-judge`** — drift *inside* each tree node. Every PRD-defined `REQ-<KEY>-<NUMBER>` has a matching REQ in the right node (`MISSING_REQ_IN_TREE`); PRD `OUT OF SCOPE` markers are honored (`STALE_OOS` / `FABRICATED_OOS`); new ACs the PRD adds to existing REQs appear in the tree (`MISSING_AC`); PRD vocabulary updates propagate (`TERMINOLOGY_DRIFT`); REQs in the tree without a PRD source are flagged (`FABRICATED_REQ`). Companion to `req-coverage-judge`'s topic-level check.
...
**Dual completion condition.** Full `pass` requires both all five reviewers pass AND `requirements/_questions-pending.md` has no open questions blocking the review. If reviewers pass on the concrete parts but open questions remain, the loop exits `awaiting_clarification` (not `fail`) — the tree is committed, the operator resolves the questions by editing `PRD.md`, and a subsequent run continues.

### 8.2 Blueprint loop (5 gates + dual completion condition)
...
- **`bp-frd-fidelity-judge`** — drift *inside* each feature blueprint. Every `REQ-<KEY>-<NUMBER>` in the FRD has architectural coverage in the blueprint (a `component` block referencing it, a System Contracts entry, a `Component Blueprint Composition` delegation, or an ADR explaining a deferral) — missing coverage is `MISSING_REQ_COVERAGE`; FRD `OUT OF SCOPE` markers honored (a blueprint still describing a working component for a deferred REQ is `STALE_OOS`); FRD vocabulary updates propagate (role renames, typed-value drift, user-story actor shifts → `TERMINOLOGY_DRIFT`). Companion to `bp-coverage-judge`'s slug-level check.

**Dual completion condition.** Same shape as the requirements loop: `pass` requires both all five reviewers pass AND `blueprints/_questions-pending.md` has zero unanswered questions. If reviewers pass but questions remain, the loop exits `awaiting_clarification` (not `fail`) — artifact is committed, state is saved, operator answers and re-triggers. One mechanism, one verdict, three file locations across the three upstream loops (`requirements/_questions-pending.md`, `blueprints/_questions-pending.md`, `work-orders/_questions-pending.md`).
...
- **`wo-coverage-judge`** — three checks: (a) every approved blueprint's delivery surface (component blocks, model blocks, feature commitments) is covered by *either* at least one work order claiming it (via a `#<blueprint-slug>` mention or a `Produces` entry) *or* by the project's existing code (the symbol exists in the repo with a shape consistent with the blueprint) — partial code coverage that doesn't fully match the blueprint requires a work order for the gap, and a surface element with neither a work order nor a code presence is `MISSING_COVERAGE`; the judge runs the same targeted codebase scan as the generator (top-level structure, entry-point files, greps for blueprint-named symbols/models/routes) to detect code presence; (b) every `#<blueprint-slug>` mention resolves to an existing file in `blueprints/{containers,components,features}/<slug>.md`; (c) for agent-executable work orders, every acceptance criterion declares an observation gate (`tests`, `playwright`, or `code-spec`) that the gate set in `## Gates` actually declares `required`. Operator-action work orders skip check (c) — their AC rows are operator-verified prose without gate tags.
