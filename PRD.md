...
- ~~**Work-order artifact shape.**~~ Resolved. Per-work-order document shape pinned: `# Title`, `## Goal`, `## Blueprints`, `## In scope`, `## Out of scope`, `## Produces` (fenced YAML), `## Depends on` (fenced YAML), `## Acceptance criteria` (`- [ ] AC-WO-<slug>.M (via tests|playwright|code-spec) — Outcome`), `## Gates` (fenced YAML), optional `## Implementation notes (non-binding)`. Full contract in `requirements/features/work-orders-loop.md` REQ-WO-002.
