...
make_fake_root() {
  ...
  # fm-worktree.sh (and the fm-tangle-lib.sh + fm-naming-lib.sh it needs):
  # teardown sources it for the plain-git worktree provider.
  ln -s "$ROOT/bin/fm-worktree.sh" "$fake/bin/fm-worktree.sh"
  ln -s "$ROOT/bin/fm-tangle-lib.sh" "$fake/bin/fm-tangle-lib.sh"
  ln -s "$ROOT/bin/fm-naming-lib.sh" "$fake/bin/fm-naming-lib.sh"
