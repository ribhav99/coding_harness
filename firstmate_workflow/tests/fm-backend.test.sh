...
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-tangle-lib.sh fm-naming-lib.sh fm-worktree.sh fm-tmux-lib.sh fm-composer-lib.sh fm-wake-lib.sh fm-classify-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-decision-hold.sh fm-backend.sh fm-operational-input.sh fm-busy-lib.sh fm-busy-event.sh fm-marker-lib.sh"
...
test_backend_name_autodetect_notice() {
  ...
  [ -s "$errfile" ] && fail "auto-detecting tmux must stay silent (the unchanged default-path behavior)"$'\n'"$(cat "$errfile")"

  pass "fm_backend_name: auto-detect selects tmux silently, and defaults to tmux with no marker"
}
...
test_backend_validate_refuses_unknown() {
  ...
  fm_backend_validate tmux-panes 2>/dev/null || fail "fm_backend_validate should accept tmux-panes"
  local out
...
test_backend_of_selector_matches_explicit_target_meta() {
  ...
  mkdir -p "$state"
  fm_write_meta "$state/panes-task.meta" "window=%12" "backend=tmux-panes"
  fm_write_meta "$state/dotfiles-d6.meta" "window=%14" "backend=tmux-panes"
  fm_write_meta "$state/fm-turnend-all-harnesses-v9.meta" "window=%16" "backend=tmux-panes"
  ...
  [ "$(fm_backend_of_selector 'dotfiles-d6' '%14' "$state")" = tmux-panes ] \
    || fail "bare non-fm task id selector should use its recorded backend"
  [ "$(fm_backend_of_selector 'fm-turnend-all-harnesses-v9' '%16' "$state")" = tmux-panes ] \
    || fail "exact fm-* task id selector should use exact metadata before legacy stripping"
  [ "$(fm_backend_of_selector 'fm-panes-task' '%12' "$state")" = tmux-panes ] \
  ...
  [ "$(fm_backend_of_selector '%12' '%12' "$state")" = tmux-panes ] \
