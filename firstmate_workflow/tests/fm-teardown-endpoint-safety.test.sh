...
test_supported_backend_endpoint_records_validate() {
  ...
  id=tmux-panes-task
  fm_write_meta "$dir/home/state/$id.meta" \
    "window=%7" "endpoint_task_id=$id" "worktree=$dir/worktree" "project=$dir/project" \
    "backend=tmux-panes"
  fm_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid tmux-panes endpoint refused"
  [ "$FM_BACKEND_VALIDATED_TARGET" = "%7" ] || fail "tmux-panes validation changed the pane identity"

  for backend in tmux tmux-panes; do
    set +e
  ...
  pass "cleanup identity: valid tmux and tmux-panes records validate while every empty backend target refuses"
}
