...
#          FM_SUPERVISOR_BACKEND    supervisor pane BACKEND (tmux;
...
#                                   falling back to
#                                   tmux). Other backends are not yet
...
# --- tunables ---------------------------------------------------------------
# Supervisor backends this daemon knows how to inject into today. Others are
# daemon has no verified composer/busy primitives wired up for them yet - see
# AGENTS.md section 4's
...
# (the 2026-07-10 overnight incident) got NO active
...
# or a captain-supplied command (push to a phone, etc.).
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  # with no equivalent elsewhere; the log line + durable marker above are already
...
#     For tmux that means a cleared composer; for a normal idle-baseline
...
inject_msg() {  # <message> [state]
  ...
  # dispatches through bin/fm-backend.sh rather than assuming tmux
  # unconditionally. Falls back to tmux
...
fm_super_main() {
  ...
  # fallback. Resolved before the target below. Exporting the result
  ...
  # tmux-specific call against it (other backends have no verified
  ...
  if ! fm_backend_list_contains "$FM_SUPERVISOR_SUPPORTED_BACKENDS" "$BACKEND"; then
    echo "error: away-mode daemon does not support supervisor backend '$BACKEND' yet (supported: $FM_SUPERVISOR_SUPPORTED_BACKENDS); set FM_SUPERVISOR_BACKEND=tmux and FM_SUPERVISOR_TARGET to run firstmate's own pane under a supported backend" >&2
  ...
  # probe, so the supervisor pane is checked through its own adapter; for
