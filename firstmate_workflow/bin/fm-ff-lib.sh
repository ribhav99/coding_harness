...
# (data/, state/, config/, projects/, .no-mistakes/), so it cannot disturb the
# backlog, projects, or in-flight work.
...
# Resolve the PRIMARY checkout's current default-branch commit. Reads the default
# branch *ref* rather than HEAD, so even a primary stranded on a feature branch
# (the worktree tangle of section 8) still yields the true default-branch tip
# instead of propagating a stray feature branch. Echoes the commit SHA, or
# returns 1.
primary_head_commit() {
...
ff_target() {
  ...
  # Read by bin/fm-update.sh after this call.
