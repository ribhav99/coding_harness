...
# script fixes. Instead this creates a non-visible tracked terminal (a tmux
# workspace with --no-focus, or a detached tmux session) that never touches the
# captain's active tab, and NEVER uses shell `&` (which a harness can reap).
#
...
#
# Supported backend: tmux. Others have no verified
...
# field (kept for the record's own documentation) is not
...
# recorded workspace id needs no separate close: closing the pane takes
