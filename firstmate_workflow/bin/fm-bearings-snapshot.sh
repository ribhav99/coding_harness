...
#
# The landed section is this home's Done, bounded by a per-home cap and an
# overall cap with omitted[] disclosure; no GitHub/network call is involved.
#
# Flags:
...
landed is this home's Done, bounded by a per-home cap (FM_BEARINGS_LANDED_PER_HOME)
  and an overall cap (FM_BEARINGS_LANDED), with omitted[] disclosure.
...
$(printf '%s' "$SNAP" | jq -r '.tasks[] | .paths.worktree.path // empty')
EOF
...
MODEL=$(printf '%s' "$SNAP" | jq \
  ...
  | ([ .backlog.records[]
         ...
         | {id,key:.id,verb:"captain-hold",
            summary:((.title + ": " + .hold_reason) | trunc(90)),owner:"(main)"} ]) as $decisions_all
  | ((if (.main_inventory.valid == false) then
     ...
     + [ .backlog.records[]
         ...
         | {id, title:(.title | trunc(60)),
            ...
            reason:((.hold_reason // .blocked_reason // "-") | trunc(40)),owner:"(main)"} ]) as $gates_all
  | ([ .scout_reports[]
       | . as $r
  ...
  | . + {omitted: (
      [ (if $f_bodies then empty else {surface:"backlog item bodies", reveal:"--fields bodies"} end),
        ...
        (if $all_landed == 0 and ($per_home_capped | length) > ($done | length) then {surface:("landed showing \($done | length) of \($per_home_capped | length)" + ""), reveal:"--all-landed"} else empty end),
