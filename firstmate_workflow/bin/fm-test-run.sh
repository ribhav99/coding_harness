...
family_for_basename() {
  case "$1" in
    ...
    fm-brief.test.sh|fm-cd-pretool-check.test.sh|\
    fm-composer-ghost.test.sh|fm-composer-lib.test.sh|\
    fm-crew-state.test.sh|fm-decision-hold-lifecycle.test.sh|\
    fm-documentation-audiences.test.sh|fm-ensure-agents-md.test.sh|fm-lint.test.sh|\
    ...
    fm-startup-memory-budget.test.sh|fm-update.test.sh)
      printf '%s\n' session-bootstrap
      ;;
...
select_lane() {
  ...
  case "$want" in
    ...
    portable-serial)
      ...
      # Watcher/lock/AFK/tmux/daemon/ambiguous/stateful work stays here,
      # serial only.
      while IFS= read -r s; do
        [ -n "$s" ] || continue
...
run_coverage_guard() {
  ...
  printf 'FM_TEST_COVERAGE ok total=%s parallel=%s serial=%s\n' \
    "$(wc -l <"$tmp/all" | tr -d ' ')" \
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-test-run.sh|bin/fm-test-isolation-proof.sh)
      printf '%s\n' pure-contract-unit
      ;;
    bin/backends/*|bin/fm-backend.sh)
      printf '%s\n' backend-dispatch
      ;;
