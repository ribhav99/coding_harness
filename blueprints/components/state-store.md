...
responsibilities:
	...
	- Loads or initialises a per-WO state file at `harness/state/<wo-slug>.json` when the coding loop selects a work order
...
responsibilities:
	...
	- For coding-loop per-WO execution, captures reviewer subprocess stdout and writes to `harness/state/reviews/coding-loop/<wo-slug>/attempt-<N>/<reviewer-name>.md`
...
fields:
	...
	- protocol_failures[]: array of incident records (reviewer name, attempt, retry count, malformed stdout)
constraints:
...
store: Filesystem (project repo, harness/state/<wo-slug>.json)
description: Per-work-order state for the coding loop's per-WO execution flow
fields:
	- wo_slug: string (`wo-<slug>`, kebab-case, stable forever; the work order's ID)
	...
	- limits: { max_wall_minutes, max_attempts }
	- execution: { branch: "task/<wo-slug>", pr_url, pr_number }
	- current: { last_output: string }
...
constraints:
	- Mirrors `.wo-<slug>.meta.yaml` for `local.*` and `status`; `local.*` refreshed on every read
	- `attempts[]` resets per invocation; `history[]` persists
	- Orchestrator is the only writer
	- `blocked_external` is mid-execution discovery; the per-WO agent sets it via #LocalPlanner and exits cleanly, the operator clears it by transitioning back to `ready`
```
...
fields:
	- path: harness/state/reviews/<loop>/[<wo-slug>/]attempt-<N>/<reviewer-name>.md
...
- **Orchestrator-only writes.** `harness/state/<loop>.json`, `harness/state/<wo-slug>.json`, and `harness/state/reviews/...` are written exclusively by the orchestrator. Generators and reviewers never touch state files.
...
- **Per-WO file path.** `harness/state/<wo-slug>.json` where `<wo-slug>` is the work order's `wo-<slug>` ID.
- **Snapshot path.** `harness/state/reviews/<loop>/attempt-<N>/<reviewer-name>.md` (upstream loops); `harness/state/reviews/coding-loop/<wo-slug>/attempt-<N>/<reviewer-name>.md` (per-WO execution).
...
- **Status enum.** `backlog | ready | in_progress | done | blocked_external` for per-WO; `running | pass | awaiting_clarification | exhausted` for per-loop.
...
**Decision.** Reviewer reviews are markdown files snapshotted into `harness/state/reviews/<loop>/[<wo-slug>/]attempt-<N>/<reviewer-name>.md`. The state JSON references attempt directories; the prose lives next door.
