...
- **Coding-loop per-work-order generator** — `claude -p "<prompt>"` with the scoped-task body (`work-orders/wo-NNN/description.md`) injected inline as context. Has full filesystem access plus `git` and `gh` access (via Bash) so it can branch, commit, push, and open the PR. Owned by @Blueprint(coding-loop).
...
- **Filesystem protocol.** Generators write only visible `<slug>.md` content files (or work-order `description.md` files). They do not write dotted-hidden meta files; #MetaMaterialiser inside the orchestrator handles those after the subprocess exits.
...
- **Hook protocol.** Hooks are configured in `.claude/settings.json` and log to `harness/logs/<task_id-or-loop-name>/hooks.log`. The Stop hook reads the chat-final-message buffer; the PreToolUse hook reads the proposed tool call and target path. Both can return non-zero to block; they do not have arbitrary side effects.
