...
container: Project Repo
responsibilities:
	- Defines the flat shape under `work-orders/`: each work order is one visible `wo-<slug>.md` content file (the scoped-task body — section shape pinned by @Feature(work-orders-loop) REQ-WO-002) plus a sibling dotted `.wo-<slug>.meta.yaml` meta file (with `id`, `status`, `priority`, `type`, `parent_id`, `blocked_by[]`, `blueprint_ids[]`)
	- Slugs are stable kebab-case identifiers; once chosen, a slug is never renamed (renaming a slug breaks every cross-reference that points to it). There are no per-WO subdirectories.
	- `work-orders/_sequence.md` is the visible execution-order list — a numbered markdown list of backticked `wo-<slug>` IDs in valid topological order, top to bottom. Mutable; the operator may re-order between loop runs. Generator-written and operator-editable; the planner walks it.
	- `work-orders/_external-blockers.md` is the operator-readable surface for every work order requiring operator action. Two sections: anticipated operator actions (`type: operator-action`) and discovered mid-execution blockers (`status: blocked_external`). Regenerated by the orchestrator on every loop-exit verdict by walking the work-orders tree's meta state. Present whenever any work order has `type: operator-action` (with `status` not `done`) OR `status: blocked_external`; absent or empty otherwise.
	- `work-orders/.sequence.meta.yaml` records the blueprint-tree hash and a generation timestamp
	- `work-orders/_inbox/wo-<slug>.md` holds gaps filed by the coding loop; the operator triages manually
	- `blocked_by[]` plus `_sequence.md` together encode everything Software Factory used phase groupings for; no additional phase field and no embedded `sort_order` field exist
```
...
responsibilities:
	...
	- `harness/state/` holds JSON state files (per-loop, per-WO) plus the reviewer review snapshot archive at `harness/state/reviews/<loop>/[<wo-slug>/]attempt-<N>/<reviewer-name>.md`
	- `harness/logs/<wo-slug-or-loop-name>/hooks.log` holds hook and session logs
...
- **Visible/hidden split.** Generators write only visible content files (`<slug>.md` in requirements/blueprints; `wo-<slug>.md` plus `_sequence.md` in work-orders). The orchestrator owns dotted-hidden meta files (`.<slug>.<kind>.meta.yaml`, `.<slug>.requirements.meta.yaml`, `.wo-<slug>.meta.yaml`, `.sequence.meta.yaml`) via `#MetaMaterialiser`, and regenerates the visible `_external-blockers.md` from current meta state.
...
- **Single source of truth for work-order dependencies.** `wo-<slug>.md`'s `Depends on.work_orders` is canonical; `.wo-<slug>.meta.yaml.blocked_by[]` is materialised from it.
...
- **State file paths.** `harness/state/<loop>.json`, `harness/state/<wo-slug>.json`, `harness/state/reviews/<loop>/[<wo-slug>/]attempt-<N>/<reviewer-name>.md`. Schemas pinned in @Blueprint(state-store).
...
**Decision.** The layout mirrors SF's entity model: nodes have content + position + parent_id + id; work orders have status + blocked_by + a stable slug ID, with execution order in a separate `_sequence.md` file. Mirrors implement push-only adapters that serialise current state.
...
### ADR-002: Flat-with-recursive-children for the requirements tree, flat slug-named files for work-orders
...
**Decision.** Requirements tree is flat-with-recursive-children: each level is the same flat shape, with `<slug>_children/` for nodes that have children. Work-orders tree is flat slug-named files: each work order is a single visible `wo-<slug>.md` file plus a sibling dotted `.wo-<slug>.meta.yaml` (no per-WO directories). Slugs are stable kebab-case identifiers; once chosen, a slug never changes.

**Consequences.** Each shape suits its consumer. Re-ordering work orders is a one-file edit (`_sequence.md`); no directory renames. Trade-off: two shapes to remember; mitigated by both being canonical and documented inline.
