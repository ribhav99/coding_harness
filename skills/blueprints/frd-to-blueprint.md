...
- **The communication folder** at `blueprints_communication/` (sibling to `blueprints/`). It holds one markdown file per reviewer in this loop (`bp-spec-judge.md`, `bp-coverage-judge.md`, `bp-consistency-judge.md`, `bp-decision-judge.md`) — append-only conversation transcripts that record each reviewer's prior reviews and your prior responses. The folder persists across orchestrator invocations forever — it is never wiped — so a re-run after any exit verdict (`pass`, `awaiting_clarification`, `exhausted`) will see the prior conversation. **Always read every reviewer file in this folder before deciding what to write or edit** — the gate is whether the file has content, not which "attempt" or "invocation" you think you're in. If a reviewer file is empty or missing, there's nothing to respond to for that reviewer yet.
