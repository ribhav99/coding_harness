...
def build_generator_resume_prompt(
    *,
    ...
    (it wrote the artifacts itself last turn) — no need to spoon-feed a diff.
    """
    failing_block = "\n".join(f"- {name}" for name in failing_reviewers) or "- (none — all reviewers passed)"
...
requirements/overview/ and requirements/features/ and run your review per
the {reviewer_name} skill.
